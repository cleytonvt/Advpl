...
Else
	...
	If !Empty(cLibok) .and. Empty(cNota) .and. Empty(cBlq) .and. Empty(cBlest) .and. Empty(cBlcred) .and. Alltrim(cContra) <> 'M'
		...
		If MsgYesNo("Deseja gerar ordem de separação para pedido: "+cNum+"?")
			...
			MsgInfo("Ordem de Separação Numero: "+cNumSep+" gerada com sucesso!","Alerta")
		...
		Else
			...
			MsgStop("Cancelado pelo usuário!","Alerta")

		EndiF
		
	Else
		
	MsgStop("Pedido não está apto para separação","Alerta")
